...
export default {
    ...
    created(){
        ...
        var walletBalance = this.openingBalanceTx.filter(function(x){ return x.balanceable_type.indexOf('Wallet') >= 0; });
        ...
        var accountBalance = this.openingBalanceTx.filter(function(x){ return x.balanceable_type.indexOf('Account') >= 0; });
    ...
    watch: {
        selectedWithdrawCurrency: function(){
            ...
            if(this.selectedWithdrawCurrency!=this.baseCurrency.ticker_default){
                var txDateUnix = moment(this.form.txDate, "DD MMMM YYYY").valueOf()
                var url = 'https://min-api.cryptocompare.com/data/dayAvg?fsym='+ this.selectedWithdrawCurrency + '&tsym=' + this.baseCurrency.ticker_default + '&toTs=' + txDateUnix
        ...
        selectedDepositCurrency: function(){
            ...
            if(this.selectedDepositCurrency!=this.baseCurrency.ticker_default){
                var txDateUnix = moment(this.form.txDate, "DD MMMM YYYY").valueOf()
                var url = 'https://min-api.cryptocompare.com/data/dayAvg?fsym='+ this.selectedDepositCurrency + '&tsym=' + this.baseCurrency.ticker_default + '&toTs=' + txDateUnix
    ...
    methods: {
        changeSource(value){
            var theValue = value.split("_")
            switch(theValue[0]){
                case "wallet" :
                    var walletBalance = this.openingBalanceTx.filter(function(x){ return x.balanceable_type.indexOf('Wallet') >= 0; });
                    var walletIdBalance = walletBalance.filter(function(x){ return x.balanceable_id == theValue[1]; });
                    this.withdrawCurrencies = walletIdBalance[0].sub_details
                    this.form.withdrawCurrencyId = walletIdBalance[0].sub_details[0].id
                    this.selectedWithdrawCurrency = walletIdBalance[0].sub_details[0].ticker_default
        ...
        changeWithdrawCurrency(value){
            var selectedCurrencyArray = this.withdrawCurrencies.filter(function(x){ return x.id == value; });
            this.selectedWithdrawCurrency = selectedCurrencyArray[0].ticker_default;
        },
        changeDepositCurrency(value){
            var selectedCurrencyArray = this.depositCurrencies.filter(function(x){ return x.id == value; });
            this.selectedDepositCurrency = selectedCurrencyArray[0].ticker_default;
        },        
        changeFee(value){
            if(value == 'percentage'){
                this.theWidth = 178
