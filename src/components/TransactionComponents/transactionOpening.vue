...
export default {
    ...
    methods: {
        addAsset(type, name, source, index){
            ...
            if($(name).length > 0){
                ...
                $(name).each(function(index){
                    ...
                    var balance = self.balance.filter(function (elm) {
                        console.log(index)
        ...
        getDecimal(id, currencies) {
            if (currencies[0]) {
                if (currencies[0].currency && currencies[0] !== undefined) {
                    var selectedCurrencyArray = currencies.filter(function(x){ return x.currency_id == id; });
                    
                    return (selectedCurrencyArray[0].currency.decimal_displayed) ? selectedCurrencyArray[0].currency.decimal_displayed : 2
                } else {
                    var selectedCurrencyArray = currencies.filter(function(x){ return x.id == id; });
                    return (selectedCurrencyArray[0].decimal_displayed) ? selectedCurrencyArray[0].decimal_displayed : 2
                }
            } else {
                return 2
            }
        ...
        savePost() {
            ...
            var dataCurrencyEmpty = 0
            var dataAmountEmpty = 0
            ...
            $.each(this.form.opening_balances, function(index, value){
                for (var i = 0; i < value.sub_details.length; i++) {
