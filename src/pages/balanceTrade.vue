...
<div class="container">
	...
	<!-- Modal -->
	<b-modal id="myModal" ref="modal" size="lg">
...
import transactionICO from '../components/TransactionComponents/transactionICO'
	...
	data() {
		return {
			...
			transactionsType: [
				...
				{ value: 'transactionICO', text: 'ICO', disabled: true },
	...
	methods: {
		loadTransaction(paramMonth){
			...
			.then(response => {
				...
				var arrayOpeningBalance = this.transactionList.find(x => x.transactionable_type == "opening_balances" )
				if(arrayOpeningBalance){
					/*
					var openingBalanceDate = new Date(arrayOpeningBalance.transaction_at)
					var currentDate = new Date(this.form.theDate)
		...
		changeTransactionType(value) {
			...
			var theMinDate = moment(this.form.theDate).startOf("month")
			var theMaxDate = moment(this.form.theDate).endOf("month")
		...
		closePeriod(){
			...
			let confirmFn = function() {
				...
				// sent data to API
				var dateObj = new Date(self.form.theDate)
