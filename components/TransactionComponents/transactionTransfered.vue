...
export default {
    ...
    watch: {
        ...
        'form.from': function (val) {
            if (val == 'outside') {
                let self = this
                let selectedCurrencyArray = this.digital_currencies.find(x => x.id == self.form.currency_id )
                let selectedTransferCurrency = selectedCurrencyArray.ticker_default
                ...
                switch(theValue[0]){
                    case "exchange" :
                        setTimeout(function(){
                            let theExchange = self.exchanges.find(x => x.id == theValue[1]);
                            self.fromAddresses = theExchange.addresses
                        }, 1000);
                        break;
                    case "wallet" :
                        setTimeout(function(){
                            let theWallet = self.wallets.find(x => x.id == theValue[1]);
                            self.fromAddresses = theWallet.addresses
                        }, 1000);
                        break;
                    default :
                        console.log(val)
            ...
            if (val == 'outside') {
                ...
                switch(theValue[0]){
                    case "exchange" :
                        setTimeout(function(){
                            let theExchange = self.exchanges.find(x => x.id == theValue[1]);
                            self.toAddresses = theExchange.addresses
                        }, 1000);
                        break;
                    case "wallet" :
                        setTimeout(function(){
                            let theWallet = self.wallets.find(x => x.id == theValue[1]);
                            self.toAddresses = theWallet.addresses
                        }, 1000);
                        break;
                    default :
                        console.log(val)
    ...
    computed: {
    	nameCurrency: function () {
    		...
    		let ticker = this.digital_currencies.filter(x => x.id == currency)
            if (ticker) {
