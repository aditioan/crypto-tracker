				...
				<b-row>
					<b-col sm="5">
                        <b-input-group id="inputAmount" size="sm">
                            <vue-autonumeric style="width:151px;" v-model.number="form.amount" :options="{decimalPlaces:decimal, minimumValue:'0' }" :state="stateAmount" :placeholder="'enter amount'"  class="text-right pr-3 form-control"></vue-autonumeric>
                            <b-form-select class="form-control" style="width:71px;" size="sm" :options="currencies" :text-field="'ticker_default'" :value-field="'id'" v-model="form.currencyid" @change="changeCurrency"></b-form-select>
                        </b-input-group>
                    </b-col>
...
export default {
    ...
    mounted(){
        if(this.dataTransactionProps != null){
            ...
            this.decimal = selectedCurrencyArray.decimal_displayed
            this.currClass = selectedCurrencyArray.classification
            ...
            this.baseCurrency = this.currencies.find(x => x.id == that.dataTransactionProps.transactionable.base_currency_id);
    ...
    watch: {
        'form.currencyid': function (val) {
            let curr = this.currencies.find(x => x.id == val)
            if (curr) {
                this.wallets = curr.supportive_wallets
            }
        },
        currClass: function(val){
            this.form.storeToId = null
        }
    },
	computed: {
		stateAmount() {
    ...
    methods: {
        ...
        getValue(value){
            if(value!=this.baseCurrency.ticker_default){
                ...
                let txDateUnix = moment(this.$parent.$parent.form.theDate, "DD MMMM YYYY").unix()
