    ...
    <div class="transactionContainer">
        ...
        <b-card no-body class="deposit" v-else>
            <form>
                    ...
                    <b-row>
                        <b-col sm="5">
                            <b-input-group id="inputBoughtAmount" size="sm">
                                <b-form-input type="text" style="width:171px;" v-model="form.boughtAmount" :state="stateBoughtAmount" placeholder="enter amount" class="text-right pr-3" v-if="form.boughtCurrencyid"></b-form-input>
                                <div style="font-size:12px; padding-top:7px; color:gray;" v-else>Select Exchange with coin supported</div>
                                <b-form-select class="form-control" style="width:65px;" size="sm" :options="exchangeCurrencies" :text-field="'ticker_default'" :value-field="'id'" v-model="form.boughtCurrencyid" v-if="form.boughtCurrencyid"></b-form-select>
                            </b-input-group>
                        </b-col>
                        <b-col sm="6" style="padding-right:0;" v-if="form.boughtCurrencyid">
                            <b-form-group id="inputTotalValueGroup" label="Price" label-for="inputValue" horizontal :invalid-feedback="feedbackBoughtValue" :state="stateBoughtValue" style="margin-bottom:0px !important;">
                        ...
                        <b-col sm="1" style="padding-left:0;" v-if="form.boughtCurrencyid">
                ...
                <b-form-group id="inputTransferAmountGroup" :label-cols="2" label="Value*" label-for="inputTransferAmount" horizontal :invalid-feedback="feedbackTransferAmount" :state="stateTransferAmount">
                    <b-row>
                        <b-col sm="5">
                        ...
                        <b-col sm="6" style="padding-right:0;">
                            <b-form-group id="inputValueGroup" label="Price" label-for="inputValue" horizontal :invalid-feedback="feedbackTransferValue" :state="stateTransferValue" style="margin-bottom:0px !important;">
                                <b-input-group size="sm">
...
export default {
    ...
    watch: {
        ...
        */
        'form.transferCurrencyid': function (val){
            let self = this
            let selectedCurrencyArray = this.fiatCurrencies.find(x => x.id == self.form.transferCurrencyid)
            this.selectedTransferCurrency = selectedCurrencyArray.ticker_default

            if (this.standart.transfer == false) {

                this.loadingTransfer = true
                let url = "https://min-api.cryptocompare.com/data/price?fsym="+this.selectedTransferCurrency+"&tsyms="+this.baseCurrency.name+""
        ...
        'form.boughtCurrencyid': function (val){
            if(this.form.boughtCurrencyid){

                let self = this
                let selectedCurrencyArray = this.currencies.find(x => x.id == self.form.boughtCurrencyid);
                this.selectedBoughtCurrency = selectedCurrencyArray.ticker_default;

                if (this.standart.bought == false) {
                    this.loadingBought = true
                    let self = this
                    let url = "https://min-api.cryptocompare.com/data/price?fsym="+this.selectedBoughtCurrency+"&tsyms="+this.baseCurrency.name+""
                    let boughtCompare = this.axios.create({
                        baseURL: url,
                        timeout: 3000,
                    });
                    boughtCompare.get()
	...
	computed: {
        ...
        nameCurrency: function () {
            ...
            let ticker = this.fiatCurrencies.find(x => x.id == currency )
            if (ticker) {
                return ticker.ticker_default
            }
        }
    ...
    methods: {
        changeExchange(value){
            ...
            this.form.boughtCurrencyid = null

            if(!value){
                value = this.form.exchangeId
            }

            let selectedExchange = this.exchanges.find(x => x.id == value);
            selectedExchange.exchange_currencies.forEach((item) => {
                this.exchangeCurrencies.push(item.currency)
            })
            if(this.exchangeCurrencies.length > 0){
                this.form.boughtCurrencyid = this.exchangeCurrencies[0].id
            }
            //this.changeDepositCurrency(this.depositCurrencies[0].id)            
        },
        countTotal(){
            if(this.form.fee == "flat") {
                this.form.paidTotal = Number(this.form.transferAmount) - Number(this.form.feeValue)
            }else if(this.form.fee == "percentage"){
                this.form.paidTotal = Number(this.form.transferAmount) - (Number(this.form.transferAmount) * (Number(this.form.feeValue)/100))
            }else {
